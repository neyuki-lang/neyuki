...
pub fn parse_assembly(text: &str) -> Result<Proto, String> {
    ...
    for raw_line in text.lines() {
        ...
        if line.starts_with(".proto") {
            ...
            let after_proto = line[".proto".len()..].trim();
        ...
        if line.starts_with(".const") {
            ...
            if parts.len() >= 4 && parts[2] == "=" {
                ...
                } else if type_and_val.starts_with("boolean ") {
                    let b = type_and_val["boolean ".len()..].trim() == "true";
                    proto.constants.push(Constant::Bool(b));
                } else if type_and_val.starts_with("int ") {
                    let s = type_and_val["int ".len()..].trim();
                    let bi = BigInt::from_str(s).unwrap_or_default();
                    proto.constants.push(Constant::Int(bi));
                } else if type_and_val.starts_with("float ") {
                    let s = type_and_val["float ".len()..].trim();
                    let f = s.parse::<f64>().unwrap_or(0.0);
                    proto.constants.push(Constant::Float(f));
                } else if type_and_val.starts_with("string ") {
                    let s = type_and_val["string ".len()..].trim();

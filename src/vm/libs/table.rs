...
fn get_table<'a>(val: &'a Value) -> Result<&'a Rc<RefCell<VmTable>>, String> {
    match val {
        Value::Table(t) => Ok(t),
...
fn table_pack(_vm: &mut VM, args: &[Value]) -> Result<Vec<Value>, String> {
...
fn table_clone(_vm: &mut VM, args: &[Value]) -> Result<Vec<Value>, String> {
    ...
    Ok(vec![Value::Table(Rc::new(RefCell::new(new_tbl)))])
}
